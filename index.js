...
const gameboard = (function(){
    let cells = [0,0,0,2,2,0,0,0,0];

    const resetBoard = cells => {cells = [0,0,0,0,0,0,0,0,0]};
    const changeCell = (index, player) => {if(cells[index] === 0){cells[index] = player} else return 'error'};
    const getBoardState = () => {return cells};
    const winCheck = (player) => {
        if( cells[0] === cells[1] === cells[2] === player || 
            cells[3] === cells[4] === cells[5] === player || 
            cells[6] === cells[7] === cells[8] === player || 
            cells[0] === cells[3] === cells[6] === player || 
            cells[1] === cells[4] === cells[7] === player || 
            cells[2] === cells[5] === cells[8] === player || 
            cells[0] === cells[4] === cells[8] === player || 
